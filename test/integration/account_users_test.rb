...
module Jumpstart
  class AccountUsersTest < ActionDispatch::IntegrationTest
    setup do
    ...
    class AdminUsers < Jumpstart::AccountUsersTest
      ...
      test 'can view account users' do
        ...
        assert_select 'a', text: I18n.t('accounts.show.edit'),
                           count: @account.account_users.count + @account.account_invitations.count
      ...
      test 'can delete account users' do
        user = users(:two)
        user = @account.account_users.find_by(user:)
